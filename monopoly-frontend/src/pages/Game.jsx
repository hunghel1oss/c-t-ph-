...
const Game = () => {
  ...
  const location = useLocation(); // Lấy gameId nếu được truyền từ Lobby
  const {
    connected,
    gameState,
    setGameState,
    ...
    onGameEnd,
    onError,
    connect: connectSocket, // Đảm bảo kết nối socket nếu chưa
  } = useGameSocket();
  
  // Lấy gameId từ state hoặc localStorage (nếu có)
  const [localGameId, setLocalGameId] = useState(location.state?.gameId || localStorage.getItem('lastGameId'));
  const [localPlayerStateId, setLocalPlayerStateId] = useState(location.state?.playerStateId);
  ...
  const [loading, setLoading] = useState(true); // Thêm trạng thái loading cục bộ
  ...
  // =========================================================================
  // ✅ FIX CHÍNH: HÀM TẢI DỮ LIỆU GAME BAN ĐẦU
  // =========================================================================
  ...
  const fetchGameData = useCallback(async (idToFetch) => {
    ...
    try {
        const response = await gameAPI.getGameInfo(idToFetch); // Dùng gameAPI để gọi GET /api/games/:gameId
        
        if (response.success) {
            setGameState(response.game);
            setLocalPlayerStateId(response.playerStateId);
            setLocalGameId(idToFetch);
            localStorage.setItem('lastGameId', idToFetch);
            
            if (!connected) {
                connectSocket(); // Đảm bảo socket được kết nối
            }
        } else {
            throw new Error(response.message || 'Failed to fetch game state');
        }
    } catch (error) {
        console.error('❌ [Game Page] Initial load error:', error);
        toast.error('Không thể tải trạng thái game. Quay về trang chủ.');
        setTimeout(() => navigate('/'), 2000); // Chuyển hướng nếu lỗi
    } finally {
        setLoading(false);
    }
  }, [setGameState, user, connected, connectSocket, navigate]);

  // ✅ INITIAL LOAD EFFECT
  useEffect(() => {
    // Ưu tiên gameId từ state/localstorage
    const idToFetch = localGameId || location.state?.gameId; 

    if (idToFetch && !gameState && loading) {
        fetchGameData(idToFetch);
    } else if (!idToFetch) {
        // Nếu không tìm thấy ID game, chuyển hướng về Home
        navigate('/');
    }

    // Tự động kết nối Socket khi trang tải
    if (user && !connected) {
      connectSocket();
    }
  }, [localGameId, gameState, loading, fetchGameData, user, connected, connectSocket, navigate, location.state]);

  // Start timer when game starts
  useEffect(() => {
    if (gameState && gameState.status === 'in_progress' && !isRunning) {
      startTimer();
    }
  ...
  useEffect(() => {
    ...
    const unsubscribeEnd = onGameEnd((data) => {
      ...
      // Giả định backend trả về playerState/squareState cho hàm calculateRankings
  ...
  const handleGameEvent = useCallback((event) => {
    switch (event.type) {
      case GAME_EVENT_TYPES.OFFER_BUY:
        // Show buy modal
        // Lấy dữ liệu từ gameState hiện tại
  ...
  const handleAction = (actionType) => {
    ...
    switch (actionType) {
      case 'buy':
        // Cần đảm bảo truyền gameId/playerStateId chính xác
  ...
  const getAvailableActions = () => {
    ...
    // Upgrade action
    if (canUpgradeProperty(player, square, gameState?.squareState)) { // Dùng optional chaining
      actions.push({
        type: 'upgrade',
        label: 'Nâng cấp',
